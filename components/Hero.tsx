        ...
        <header className="py-4 px-4 sm:py-6 sm:px-20 md:px-32 bg-white bg-opacity-10 backdrop-blur-md">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-4">
              <Image src="/image1.jpg" width={40} height={40} className="rounded-full sm:w-14 sm:h-14" alt="Holiges Logo" />
              <span className="text-2xl sm:text-4xl font-bold font-nightydemo text-yellow-300">Holiges</span>
            </div>
            <nav className="hidden md:flex space-x-4 lg:space-x-6">
              {['Home', 'About', 'Tournaments', 'Gallery', 'Blogs'].map((item) => (
                <Link key={item} href={`/${item.toLowerCase()}`}>
                  <span className="text-sm lg:text-base hover:text-yellow-300 transition-colors">{item}</span>
                </Link>
              ))}
            </nav>
            ...
            >
              <ChevronDown size={24} className={`transition-transform ${isMenuOpen ? 'rotate-180' : ''}`} />
            </button>
          </div>
          {isMenuOpen && (
            <motion.nav 
            ...
            >
              {['Home', 'About', 'Tournaments', 'Gallery', 'Blogs'].map((item) => (
                <Link key={item} href={`/${item.toLowerCase()}`}>
                  <span className="block py-2 hover:text-yellow-300 transition-colors">{item}</span>
                </Link>
              ))}
              <Link href="/play">
        ...
        <main className="mx-auto px-4 sm:px-20 md:px-32 py-8 sm:py-12">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
            <div className="w-full md:w-1/2 flex flex-col">
              ...
              >
                Challenge yourself with Holiges, the ultimate soccer trivia game. Climb the ranks from novice to legend!
              </motion.p>
              <motion.div 
            ...
            >
              <div className="relative h-64 sm:h-80 md:h-96 rounded-3xl overflow-hidden shadow-xl">
                <Image src="/imageThree.png" layout="fill" objectFit="cover" alt="Soccer Trivia" />
                <div className="absolute inset-0 bg-purple-700 bg-opacity-70 flex items-center justify-center p-4">
                  <div className="text-center">
                    <h2 className="text-xl sm:text-2xl font-bold mb-2 sm:mb-4 text-yellow-300">Sample Question:</h2>
                    <p className="text-base sm:text-xl italic text-white">{sampleQuestions[currentQuestion]}</p>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          <div className="mt-16 sm:mt-20">
            <h2 className="text-2xl sm:text-3xl font-bold mb-6 sm:mb-8 text-center text-yellow-300">Why Play Holiges?</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {[
                { icon: Trophy, title: "Climb the Ranks", description: "Start as a Baby Fan and work your way up to Hooligan status!" },
                { icon: Users, title: "Compete Globally", description: "Challenge players from around the world and prove your soccer knowledge." },
                { icon: Clock, title: "Daily Challenges", description: "New questions every day to keep you on your toes and expand your knowledge." },
              ].map((feature, index) => (
                <motion.div 
                  key={index}
                ...
                >
                  <feature.icon className="mx-auto mb-4 sm:mb-6 text-yellow-300" size={48} />
                  <h3 className="text-lg sm:text-xl font-semibold mb-2 text-yellow-300">{feature.title}</h3>
