...
import { StadiumItem } from "@/const/type";
...
const Play = () => {
  ...
  useEffect(() => {
    if (stadiumList.length > 0) {
      ...
      for (let level = 1; level <= 5; level++) {
        const filteredLevelStadiums = stadiumList.filter(
          (stadium) => stadium.level === level
        );
        const questions = filteredLevelStadiums
          .sort(() => 0.5 - Math.random())
          .slice(0, 5)
          .map((stadium) => {
            const wrongAnswers = stadiumList
              .filter(
                (item) => item.title !== stadium.title && item.level === level
              )
              .sort(() => 0.5 - Math.random())
              .slice(0, 3)
              .map((item) => item.title);
            ...
            return {
              ...
              question: "Which football team does the stadium you see belong to?",
              answers: [stadium.title, ...wrongAnswers].sort(
                () => 0.5 - Math.random()
              ),
              correctAnswer: stadium.title,
              level: stadium.level,
              title: stadium.name
            };
          });
  ...
  const { imageUrl, question, answers, correctAnswer, level, title } =
      ...
      >
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col items-center justify-center mb-6">
            <h1 className="text-black py-2 font-mono text-xl">{title}</h1>
            <img
              src={imageUrl}
              alt="Stadium"
              className="w-80 h-full object-contain rounded-xl"
            />
          </div>
